...
def git_hub_pr_summarizer(log: AxiomLogger, secrets: AxiomSecrets, input: PRDetails) -> PRSummary:
    ...
    if not ok:
        log.error("git_hub_pr_summarizer: ANTHROPIC_API_KEY secret not found")
    ...
    log.info("git_hub_pr_summarizer: done", summary_len=len(summary))
